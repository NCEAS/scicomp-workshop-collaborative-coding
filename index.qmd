...
If you do need to create an SSH key pair you can do so via RStudio. Click the "Tools" menu option then "Global Options..." then "Git/SVN" and finally "Create RSA Key...". You will be prompted to enter a passphrase that will be required to access your SSH key later on. This step is technically optional but is considered a best practice.

When you're ready, click "Create" and RStudio will generate the key pair and store them in the ".ssh" folder of your computer as "id_rsa" and "id_rsa.pub" for the private vs. public keys respectively.
...
At the end of the above steps your token should be set! For additional information, see . If this seems a little intimidating, please keep in mind that your experience with downloading programs and creating web accounts will help with these tasks even if you have more minimal coding experience!
